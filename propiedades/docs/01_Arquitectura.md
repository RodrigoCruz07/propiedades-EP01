# Propiedades — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `PropiedadApplication` | Punto de entrada Spring Boot |
| `controller/PropiedadController.java` | API REST expuesta en `/api/propiedads` |
| `service/PropiedadService.java` | Lógica de negocio y transacciones |
| `repository/PropiedadRepository.java` | Acceso JPA a H2 |
| `model/Propiedad.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → PropiedadController → PropiedadService → PropiedadRepository → H2`
