package cl.duoc.jv0101.caso11.propiedades.cucumber;
...
public class PropiedadSteps {
    ...
    private Long id;
    private ResponseEntity<Propiedad> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/propiedads";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Propiedad[]> listado = rest.getForEntity(url(), Propiedad[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Propiedad> creado = rest.postForEntity(
                url(), body(nombre), Propiedad.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Propiedad.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Propiedad.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Propiedad> noExiste = rest.getForEntity(url() + "/" + id, Propiedad.class);
